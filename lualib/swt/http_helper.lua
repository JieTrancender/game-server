...
local function upgrade(handle_agent, request)
    skynet.error("upgrade", handle_agent, request, request.id, request.addr)
    ...
    skynet.error("upgrade", handle_agent, request, ok, err, handle_agent.error)
...
local function dispatch(router, id, addr)
    ...
    skynet.error("http_helper dispatch", code, url, method, headers, body)
    if not code then
    ...
    skynet.error("http_helper dispatch", method, path, id, addr, path, method, query, body)
    ...
    skynet.error("http_helper dispatch execute result", ok, error)
    if not ok then
